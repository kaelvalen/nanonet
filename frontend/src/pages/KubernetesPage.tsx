...
export function KubernetesPage() {
	...
	const {
		...
		enabled:
			isAvailable && (activeTab === "services" || activeTab === "endpoints"),
		retry: 1,
		refetchInterval: 20000,
	});
	...
	const {
		...
		queryFn: () =>
			k8sApi.getEvents(eventKindFilter === "all" ? undefined : eventKindFilter),
	...
	// NanoNet servisleri
	const {
		data: nanonetServicesData,
		isLoading: nanonetServicesLoading,
		refetch: refetchNanonetServices,
	} = useQuery({
	...
	const scaleMutation = useMutation({
		mutationFn: ({ name, replicas }: { name: string; replicas: number }) =>
			k8sApi.scaleDeployment(name, replicas),
		onSuccess: (result) => {
	...
	const hpaMutation = useMutation({
		mutationFn: () =>
			k8sApi.createOrUpdateHPA(hpaDeployment, hpaMin, hpaMax, hpaCpu),
		onSuccess: () => {
	...
	const deployMutation = useMutation({
		...
		onSuccess: (res, variables) => {
			...
			setDeployForms((prev) => ({
				...prev,
				[variables.name]: {
					...(prev[variables.name] ?? { image: "", replicas: 1, open: false }),
					open: false,
				},
			}));
			queryClient.invalidateQueries({ queryKey: ["k8s-deployments-list"] });
		...
		onError: (err: unknown) => {
			const apiMsg = (err as { response?: { data?: { error?: string } } })
				?.response?.data?.error;
	...
	const filteredPods = pods.filter(
		(p) =>
			...
			Object.values(p.labels ?? {}).some((v) =>
				v.toLowerCase().includes(podFilter.toLowerCase()),
			),
	);
	const runningPods = pods.filter((p) => p.status === "Running").length;
	const readyDeployments = deployments.filter(
		(d) => d.ready_replicas === d.replicas && d.replicas > 0,
	).length;
