...
function statusColors(s: string) {
	if (s === "up") return { bg: "var(--status-up-subtle)", text: "var(--status-up-text)", dot: "var(--status-up)" };
	if (s === "degraded") return { bg: "var(--status-warn-subtle)", text: "var(--status-warn-text)", dot: "var(--status-warn)" };
	return { bg: "var(--status-down-subtle)", text: "var(--status-down-text)", dot: "var(--status-down)" };
}

function severityColors(sev: string) {
	if (sev === "crit") return { bg: "var(--status-down-subtle)", text: "var(--status-down-text)", border: "var(--status-down-border)", dot: "var(--status-down)", label: "Kritik" };
	if (sev === "warn") return { bg: "var(--status-warn-subtle)", text: "var(--status-warn-text)", border: "var(--status-warn-border)", dot: "var(--status-warn)", label: "Uyarı" };
	return { bg: "var(--color-teal-subtle)", text: "var(--color-teal)", border: "var(--color-teal-border)", dot: "var(--color-teal)", label: "Bilgi" };
}
...
export function DashboardPage() {
	...
	const displayServices  = criticalServices.length > 0 ? criticalServices : services.slice(0, 8);

	const healthPercent = totalServices > 0 ? Math.round((onlineServices / totalServices) * 100) : 0;
	const healthColor   = healthPercent >= 90 ? "var(--status-up)" : healthPercent >= 70 ? "var(--status-warn)" : "var(--status-down)";
	...
	const genSparkline = (base: number, variance: number) =>
		Array.from({ length: 12 }, () => Math.max(0, base + (Math.random() - 0.5) * variance * 2));
		...
		return (
			...
			>
				<div className="w-full max-w-lg text-center">
					<div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-5" style={{ background: "var(--gradient-logo)" }}>
						<Server className="w-8 h-8 text-white" />
					</div>
					<h1 className="text-2xl font-bold mb-2" style={{ color: "var(--text-primary)" }}>NanoNet'e Hoş Geldiniz</h1>
					...
					<div className="grid grid-cols-3 gap-3 mb-8">
						{[
							...
							<div key={item.step} className="p-4 rounded-xl text-left" style={{ background: "var(--surface-card)", border: "1px solid var(--border-default)" }}>
								<div className="w-7 h-7 rounded-lg flex items-center justify-center mb-3 text-xs font-bold" style={{ background: "var(--color-teal-subtle)", color: "var(--color-teal)", border: "1px solid var(--color-teal-border)" }}>
									{item.step}
								</div>
	...
	return (
		<div className="space-y-3">
			...
			>
				...
				<div className="flex items-center gap-2 flex-wrap">
					<span
						...
						style={{
							background: healthPercent === 100 ? "var(--status-up-subtle)" : healthPercent >= 70 ? "var(--status-warn-subtle)" : "var(--status-down-subtle)",
							color: healthPercent === 100 ? "var(--status-up-text)" : healthPercent >= 70 ? "var(--status-warn-text)" : "var(--status-down-text)",
				...
				<div className="flex items-center gap-2">
					<button
						...
						style={{ background: "var(--color-teal-subtle)", color: "var(--color-teal)", border: "1px solid var(--color-teal-border)" }}
					>
					...
					<button
						...
						className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all hover:opacity-80"
						style={{ background: "var(--color-lavender-subtle)", color: "var(--color-lavender)", border: "1px solid var(--color-lavender-border)" }}
					>
			...
			<div className="grid grid-cols-12 gap-3">
				...
				{([
					{ label: "Toplam",     value: totalServices,    icon: Server,        color: "var(--color-teal)",  bg: "var(--color-teal-subtle)",  border: "var(--color-teal-border)",  to: "/app/services" },
					{ label: "Çevrimiçi", value: onlineServices,   icon: CheckCircle2,  color: "var(--status-up)",   bg: "var(--status-up-subtle)",   border: "var(--status-up-border)",   to: "/app/services" },
					{ label: "Bozuk",     value: degradedServices,  icon: AlertTriangle, color: "var(--status-warn)", bg: "var(--status-warn-subtle)", border: "var(--status-warn-border)", to: "/app/services" },
					{ label: "Offline",   value: offlineServices,   icon: XCircle,       color: "var(--status-down)", bg: "var(--status-down-subtle)", border: "var(--status-down-border)", to: "/app/services" },
				] as const).map((s, i) => (
					<motion.div
						key={s.label}
					...
					>
						<Link to={s.to}>
							<Card className="relative overflow-hidden px-4 py-3.5 h-full hover:shadow-md transition-shadow" style={{ background: "var(--surface-card)", border: "1px solid var(--border-default)" }}>
								<div className="absolute left-0 top-3 bottom-3 w-0.5 rounded-r-full" style={{ background: s.color }} />
								<div className="flex items-center justify-between gap-2">
									<div>
										<p className="text-[11px] font-medium mb-1" style={{ color: "var(--text-faint)" }}>{s.label}</p>
										<p className="text-3xl font-bold tabular-nums leading-none" style={{ color: "var(--text-primary)" }}>
											{isLoading
												? <span className="inline-block w-8 h-7 rounded animate-pulse" style={{ background: "var(--surface-sunken)" }} />
												: s.value}
										</p>
									</div>
				...
				{([
					{
						...
						trend: globalSummary?.avg_latency_ms && globalSummary.avg_latency_ms < 100,
					},
					{
						label: "Ort. CPU",
						value: fmt(globalSummary?.avg_cpu_percent, "%"),
						sub: globalSummary?.avg_cpu_percent && globalSummary.avg_cpu_percent > 80 ? "Yüksek kullanım" : "Normal",
						icon: Cpu,
						color: "var(--color-teal)",
						sparkline: cpuSparkline,
						trend: globalSummary?.avg_cpu_percent ? globalSummary.avg_cpu_percent < 70 : true,
					},
					{
						label: "Bellek",
						value: globalSummary?.avg_memory_used_mb && globalSummary.avg_memory_used_mb > 1024
					...
					>
						<Card className="px-4 pt-3.5 pb-2 h-full" style={{ background: "var(--surface-card)", border: "1px solid var(--border-default)" }}>
							<div className="flex items-start justify-between mb-1">
								<div>
									<p className="text-[11px] font-medium" style={{ color: "var(--text-faint)" }}>{m.label}</p>
									<p className="text-2xl font-bold tabular-nums font-mono leading-tight mt-0.5" style={{ color: "var(--text-primary)" }}>
										{globalSummary ? m.value : <span className="inline-block w-14 h-6 rounded animate-pulse" style={{ background: "var(--surface-sunken)" }} />}
									</p>
								</div>
								<div className="flex flex-col items-end gap-1">
									<div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "var(--surface-sunken)" }}>
										<m.icon className="w-3.5 h-3.5" style={{ color: m.color }} />
									</div>
									{m.trend
										? <TrendingUp className="w-3 h-3" style={{ color: "var(--status-up)" }} />
										: <TrendingDown className="w-3 h-3" style={{ color: "var(--status-down)" }} />}
								</div>
							</div>
							<Sparkline data={m.sparkline} color={m.color} />
							<p className="text-[10px] mt-0.5" style={{ color: "var(--text-faint)" }}>{m.sub}</p>
						</Card>
					</motion.div>
				))}
				...
				>
					<Card className="px-5 py-3" style={{ background: "var(--surface-card)", border: "1px solid var(--border-default)" }}>
						<div className="flex items-center gap-4">
							...
							<div className="flex-1">
								<div className="flex justify-between mb-1.5">
									...
									<span className="text-[11px] font-bold tabular-nums font-mono" style={{ color: healthColor }}>{healthPercent}%</span>
								</div>
							...
							<div className="hidden sm:flex items-center gap-4 shrink-0">
								{[
									{ label: "Aktif", val: onlineServices, color: "var(--status-up)" },
									{ label: "Bozuk", val: degradedServices, color: "var(--status-warn)" },
									{ label: "Offline", val: offlineServices, color: "var(--status-down)" },
								].map((seg) => (
									<div key={seg.label} className="flex items-center gap-1.5">
										<span className="w-2 h-2 rounded-full" style={{ background: seg.color }} />
										<span className="text-xs font-mono tabular-nums font-semibold" style={{ color: "var(--text-secondary)" }}>{seg.val}</span>
										<span className="text-[10px]" style={{ color: "var(--text-faint)" }}>{seg.label}</span>
									</div>
								))}
							</div>
						</div>
					...
					<Card className="p-4" style={{ background: "var(--surface-card)", border: "1px solid var(--border-default)" }}>
						...
						<div className="flex items-center justify-between mb-3">
							<div className="flex items-center gap-2">
								...
								<span className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>
									{criticalServices.length > 0 ? "Dikkat Gereken Servisler" : "Servisler"}
								</span>
							...
							<Link to="/app/services" className="flex items-center gap-1 text-xs hover:opacity-70 transition-opacity" style={{ color: "var(--text-faint)" }}>
						...
						<div className="space-y-1">
									...
									return (
										...
										>
											...
											>
												...
												{/* Host */}
												<span className="text-[11px] font-mono hidden sm:block shrink-0" style={{ color: "var(--text-faint)" }}>
													...
													<div className="hidden lg:flex items-center gap-1.5 shrink-0 w-24">
														<div className="flex-1 h-1 rounded-full overflow-hidden" style={{ background: "var(--border-subtle)" }}>
															<div
																...
																style={{
																	width: `${uptime}%`,
																	background: uptime >= 99 ? "var(--status-up)" : uptime >= 95 ? "var(--status-warn)" : "var(--status-down)",
																}}
															/>
														</div>
														<span
															className="text-[10px] font-mono tabular-nums w-9 text-right"
															style={{ color: uptime >= 99 ? "var(--status-up-text)" : uptime >= 95 ? "var(--status-warn-text)" : "var(--status-down-text)" }}
														>
															{uptime.toFixed(1)}%
														</span>
													</div>
												...
												{/* Status badge */}
												<Badge className="text-[10px] px-1.5 shrink-0 border-0" style={{ background: sc.bg, color: sc.text }}>
							...
							{criticalServices.length === 0 && services.length > 8 && (
								<Link to="/app/services" className="flex items-center justify-center gap-1 py-2 text-xs hover:opacity-70" style={{ color: "var(--text-faint)" }}>
									+{services.length - 8} servis daha <ArrowRight className="w-3 h-3" />
								</Link>
							)}
						</div>
					</Card>
					...
					<Card className="p-4" style={{ background: "var(--surface-card)", border: "1px solid var(--border-default)" }}>
						...
						<div className="flex items-center justify-between mb-3">
							<div className="flex items-center gap-2">
								...
								<span className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Aktif Uyarılar</span>
								{recentAlerts.length > 0 && (
									<div className="flex items-center gap-1">
										...
										{warnAlerts > 0 && (
											<span className="text-[10px] font-bold px-1.5 py-0.5 rounded-md" style={{ background: "var(--status-warn-subtle)", color: "var(--status-warn-text)", border: "1px solid var(--status-warn-border)" }}>
												{warnAlerts} uyarı
											</span>
										)}
									</div>
							...
							<Link to="/app/alerts" className="flex items-center gap-1 text-xs hover:opacity-70 transition-opacity" style={{ color: "var(--text-faint)" }}>
								...
								{recentAlerts.map((alert) => {
									...
									return (
										<div
											...
											style={{ background: "var(--surface-sunken)", borderLeft: `3px solid ${sv.dot}` }}
										>
											{/* Severity dot */}
											...
											<div className="flex-1 min-w-0">
												<div className="flex items-center gap-1.5 mb-0.5">
													...
													<span className="text-[10px] font-bold px-1 py-0.5 rounded shrink-0" style={{ background: sv.bg, color: sv.text, border: `1px solid ${sv.border}` }}>
														{sv.label}
													</span>
												</div>
