...
export function ServicesPage() {
	...
	const slaOk = useMemo(
		() => Object.values(uptimeMap).filter((v) => v >= 99.9).length,
		[uptimeMap],
	);
	const slaWarn = useMemo(
		() => Object.values(uptimeMap).filter((v) => v >= 95 && v < 99.9).length,
		[uptimeMap],
	);
	const slaCrit = useMemo(
		() => Object.values(uptimeMap).filter((v) => v < 95).length,
		[uptimeMap],
	);
		...
		switch (status) {
			case "up":
				return {
					...
					badgeBorder: "var(--status-up-border)",
				...
				<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
					<div>
						...
						<p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
							Tüm mikroservisleri izle ve yönet
						</p>
					</div>
					<AddServiceDialog />
				</div>
								...
								(s) => (
									...
									>
										{s === "all"
											...
											: s === "up"
												? `Aktif (${statusCounts.up})`
												: s === "degraded"
													? `Bozuk (${statusCounts.degraded})`
													: s === "down"
			...
			{isLoading ? (
				...
				>
					...
					<p className="text-xs" style={{ color: "var(--text-faint)" }}>
						{services.length === 0
							? 'Başlamak için "Servis Ekle" butonuna tıklayın'
											...
											<div className="flex items-start justify-between mb-3">
												<div className="flex items-center gap-3 min-w-0">
													...
													<div className="min-w-0">
														...
														<p
															className="text-[10px] font-(--font-mono) truncate"
															style={{ color: "var(--text-faint)" }}
														>
												...
												<Badge
													className="text-[9px] font-(--font-mono) px-2 py-0.5 rounded border-2 shrink-0"
													style={{
														background: sv.badgeBg,
														color: sv.badgeText,
														borderColor: sv.badgeBorder,
													}}
												>
												...
												<div className="flex items-center gap-2">
													{uptimeMap[service.id] != null && (
														<span
															...
															style={{
																color:
																	uptimeMap[service.id] >= 99
																		? "var(--status-up-text)"
																		: uptimeMap[service.id] >= 95
																			? "var(--status-warn-text)"
																			: "var(--status-down-text)",
															}}
														>
									...
									<Link to={`/services/${service.id}`} className="block group">
										...
										>
											<div className="flex items-center gap-4">
												...
												<span
													className="text-[10px] hidden sm:flex items-center gap-1"
													style={{ color: "var(--text-faint)" }}
												>
													<Clock className="w-3 h-3" />
													{service.poll_interval_sec}s
												...
												<Badge
													className="text-[9px] font-(--font-mono) px-1.5 py-0.5 rounded border-2 shrink-0"
													style={{
														background: sv2.badgeBg,
														color: sv2.badgeText,
														borderColor: sv2.badgeBorder,
													}}
												>
													{service.status?.toUpperCase() ?? "UNKNOWN"}
												</Badge>
