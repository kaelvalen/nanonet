...
import { metricsApi, type Alert, type AlertRules } from "@/api/metrics";
...
function AlertRulesPanel() {
  ...
  const effectiveDraft: RulesDraft = draft ?? (rules
    ? { cpu_threshold: rules.cpu_threshold, memory_threshold_mb: rules.memory_threshold_mb, latency_threshold_ms: rules.latency_threshold_ms, error_rate_threshold: rules.error_rate_threshold }
    : { cpu_threshold: 80, memory_threshold_mb: 1024, latency_threshold_ms: 1000, error_rate_threshold: 0.1 }
  );

  const saveMutation = useMutation({
    mutationFn: () => metricsApi.updateAlertRules(selectedServiceId!, effectiveDraft),
    onSuccess: () => {
      toast.success("Eşik değerleri kaydedildi");
      queryClient.invalidateQueries({ queryKey: ["alert-rules", selectedServiceId] });
      refetchRules();
    },
  ...
  const isDirty = draft !== null && rules !== undefined && (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {servicesLoading ? (
          <div className="flex items-center gap-2 text-xs" style={{ color: "var(--text-faint)" }}>
            <Loader2 className="w-3.5 h-3.5 animate-spin" />Servisler yükleniyor...
          </div>
        ) : services.length === 0 ? (
          <p className="text-xs" style={{ color: "var(--text-faint)" }}>Henüz servis yok.</p>
        ) : (
          services.map((svc) => (
            <button key={svc.id} onClick={() => {
              ...
              style={selectedServiceId === svc.id
                ? { background: "color-mix(in srgb, var(--color-teal) 12%, transparent)", borderColor: "var(--color-teal-border)", color: "var(--color-teal)" }
                : { color: "var(--text-muted)", borderColor: "var(--border-subtle)", background: "transparent" }}
            >
              <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{
                backgroundColor: svc.status === "up" ? "var(--status-up)" : svc.status === "down" ? "var(--status-down)" : "var(--status-warn)"
              }} />
              {svc.name}
            </button>
          ))
      ...
      {!selectedServiceId && (
        <Card className="p-10 rounded text-center" style={{ background: "var(--surface-card)", border: "2px solid var(--border-default)", boxShadow: "var(--card-shadow)" }}>
          <Settings2 className="w-8 h-8 mx-auto mb-3 opacity-25" style={{ color: "var(--text-faint)" }} />
          <p className="text-sm font-medium" style={{ color: "var(--text-muted)" }}>Eşik ayarlamak için bir servis seçin</p>
          <p className="text-xs mt-1" style={{ color: "var(--text-faint)" }}>Her servis bağımsız threshold değerleri kullanabilir</p>
        </Card>
      )}
      ...
      {selectedServiceId && (
        <>
          {rulesLoading ? (
            <div className="flex items-center gap-2 py-6 text-xs" style={{ color: "var(--text-faint)" }}>
              <Loader2 className="w-4 h-4 animate-spin" />Eşik değerleri yükleniyor...
            </div>
          ) : (
            <div className="space-y-3">
              {rules?.is_default === true && (
                <div className="flex items-center gap-2 px-3 py-2 rounded text-xs" style={{ background: "var(--color-blue-subtle)", border: "2px solid var(--color-blue-border)", color: "var(--color-blue-text)" }}>
                  <Info className="w-3.5 h-3.5 shrink-0" />
                  Bu servis şu an global varsayılan eşik değerlerini kullanıyor.
                </div>
              )}
              {fields.map((f) => (
                <Card key={f.key} className="p-4 rounded" style={{ background: "var(--surface-card)", border: "2px solid var(--border-default)", boxShadow: "var(--card-shadow)" }}>
                  <div className="flex items-center gap-2 mb-3">
                    <div className="w-8 h-8 rounded flex items-center justify-center shrink-0"
                      style={{ border: `1.5px solid ${f.color}`, backgroundColor: `color-mix(in srgb, ${f.color} 12%, transparent)` }}>
                      <f.icon className="w-4 h-4" style={{ color: f.color }} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold" style={{ color: "var(--text-secondary)" }}>{f.label}</p>
                      <p className="text-[10px] truncate" style={{ color: "var(--text-faint)" }}>{f.hint}</p>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <Input
                        ...
                        style={{ background: "var(--input-bg)", borderColor: `color-mix(in srgb, ${f.color} 40%, var(--input-border))`, color: "var(--text-secondary)" }}
                      />
                      <span className="text-[10px] w-6" style={{ color: "var(--text-faint)" }}>{f.unit}</span>
                    </div>
                  </div>
                  <Slider
                    value={[f.value]}
                  ...
                  <div className="flex justify-between text-[9px] mt-1" style={{ color: "var(--text-faint)" }}>
                    ...
                    <span style={{ color: f.color, fontWeight: 600 }}>{f.key === "error_rate_threshold" ? (f.value * 100).toFixed(0) + "%" : f.value + f.unit}</span>
              ...
              <div className="flex items-center gap-2 justify-end pt-1">
                {isDirty && (
                  <button onClick={() => { setDraft(null); }}
                    ...
                    <RefreshCw className="w-3.5 h-3.5 inline mr-1" />Sıfırla
                  </button>
                )}
                <Button
                  ...
                  style={{ background: isDirty ? "var(--color-teal)" : "var(--text-faint)" }}
                >
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }}>
          <Card className="p-14 rounded text-center" style={{ background: "var(--surface-card)", border: "2px solid var(--status-up-border)", boxShadow: "var(--card-shadow)" }}>
            <div className="w-16 h-16 rounded flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: "var(--status-up-subtle)", border: "2px solid var(--status-up-border)" }}>
              ...
              {filtered.map((alert, index) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      {/* Timeline icon */}
                      <div className="relative z-10 w-9 h-9 rounded flex items-center justify-center border-2 shrink-0 mt-0.5"
                      ...
                      {/* Card */}
                      <Card className={`flex-1 rounded p-4 transition-all duration-200 ${alert.resolved_at ? "opacity-60" : ""}`}
                        style={{ background: "var(--surface-card)", border: `2px solid ${config.borderVar}`, boxShadow: "var(--card-shadow)" }}>
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                              <Badge className="text-[9px] px-1.5 py-0 rounded border-2 uppercase font-(--font-mono)"
                              ...
                              {alert.resolved_at && (
                                <Badge className="text-[9px] px-1.5 py-0 rounded border-2"
