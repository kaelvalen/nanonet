          ...
          <Card className="backdrop-blur-xl rounded-2xl p-8" style={{ background: "var(--surface-glass)", border: "1px solid var(--color-teal-border)", boxShadow: "0 20px 40px var(--shadow-card)" }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.38 }} className="grid gap-2">
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 transition-colors" style={{ color: "var(--text-faint)" }}>
                    ...
                    <div className="flex gap-1 flex-1">
                      {[1, 2, 3].map((level) => (
                        <div key={level} className="h-1 flex-1 rounded-full transition-all duration-300"
                          style={{ backgroundColor: passwordStrength >= level ? strengthColor : "var(--border-subtle)" }} />
                      ))}
                    </div>
              ...
              <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.46 }} className="grid gap-2">
                ...
                <Input
                  ...
                  required
                />
                {passwordMismatch && <p className="text-[10px]" style={{ color: "var(--status-down-text)" }}>Passwords don't match</p>}
                {passwordsMatch && <p className="text-[10px]" style={{ color: "var(--status-up-text)" }}>Passwords match ✓</p>}
              </motion.div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.62 }} className="mt-6 text-center">
              <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                ...
                <Link to="/login" className="font-medium transition-colors" style={{ color: "var(--text-link)" }}>
                  Sign In
                </Link>
              </p>
            </motion.div>
