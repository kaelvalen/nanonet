		...
		<PageShell>
			<PageHeader
				...
				actions={
					<Button
						onClick={() =>
							setDraft({ name: "", scopes: ["services:read"], expires_in_days: 0 })
						}
						className="gap-2 h-9"
					>
						<Plus className="size-4" /> Yeni Token
					</Button>
				}
			/>
			...
			{isLoading ? (
				...
				>
					...
					>
						Henüz API token oluşturulmadı
					</div>
					<div
						className="mt-1 text-xs"
						style={{ color: "var(--text-faint)" }}
					>
						Yukarıdaki "Yeni Token" düğmesiyle ilkini oluştur.
					</div>
				</div>
			) : (
			...
			<div className="flex items-start gap-3">
				...
				<div className="flex-1">
					...
					<div className="mt-3 flex items-center gap-2">
						<code
							className="flex-1 rounded px-3 py-2 text-xs font-mono break-all"
							style={{
...
function DraftEditor({
	...
	return (
		<div
			className="mb-4 rounded border p-5"
			style={{
				background: "var(--card-bg)",
				borderColor: "var(--border-subtle)",
			}}
				...
				<div className="mt-2 grid gap-1 sm:grid-cols-2 lg:grid-cols-3">
					{availableScopes.map((scope) => {
						...
						return (
							...
							>
								{checked ? (
									<CheckCircle2 className="mt-0.5 size-3.5 text-indigo-400" />
								) : (
									<div
		...
		>
			<div className="min-w-0 flex-1">
				<div className="flex items-center gap-2">
					...
					{!revoked && expired && (
						<span className="inline-flex items-center gap-1 rounded bg-amber-500/15 px-2 py-0.5 text-[10px] font-medium text-amber-300">
							Süresi dolmuş
						</span>
					)}
				</div>
				...
				<div className="mt-2 flex flex-wrap gap-1">
					{token.scopes.map((s) => (
						<span
							key={s}
							className="rounded bg-white/5 px-1.5 py-0.5 text-[10px] font-mono"
							style={{ color: "var(--text-secondary)" }}
						>
							{s}
						</span>
