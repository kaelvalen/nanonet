		...
		<div className="space-y-5">
			...
			{allInsights.length > 0 && (
				...
				>
					{[
						...
						{ label: "Yüksek", value: statHigh, icon: AlertTriangle, color: "var(--status-down)", bg: "var(--status-down-subtle)", border: "var(--status-down-border)" },
						{ label: "Orta", value: statMedium, icon: TrendingUp, color: "var(--status-warn)", bg: "var(--status-warn-subtle)", border: "var(--status-warn-border)" },
						{ label: "Düşük", value: statLow, icon: CheckCircle2, color: "var(--status-up)", bg: "var(--status-up-subtle)", border: "var(--status-up-border)" },
						...
						<div className="flex flex-1 items-center gap-2 flex-wrap">
							...
							>
								{analyzeMutation.isPending ? (
									<><RefreshCw className="w-3.5 h-3.5 mr-1.5 animate-spin" /> Analiz ediliyor...</>
								) : (
			...
			{analyzeMutation.isPending && (
				<Card
					...
					style={{ background: "var(--surface-card)", border: "1px solid var(--color-lavender-border)" }}
				>
					<div className="flex items-center gap-2">
						<Brain className="w-4 h-4 animate-pulse" style={{ color: "var(--color-lavender)" }} />
						<div className="h-3 w-40 rounded animate-pulse" style={{ background: "var(--color-lavender-subtle)" }} />
					</div>
					{[80, 55, 90, 40].map((w) => (
						<div
							key={w}
						...
						>
							...
							<div className="p-5 space-y-4">
								...
								<div className="flex items-center gap-3">
									...
									>
										<Brain className="w-4.5 h-4.5" style={{ color: "var(--color-lavender)" }} />
									</div>
									<div className="flex-1">
										...
										<p className="text-xs" style={{ color: "var(--text-faint)" }}>
											{deepAnalysis ? "Derin analiz" : "Hızlı analiz"} · Son 30 dakika
										</p>
									</div>
										...
										<div className="flex items-center gap-2 shrink-0">
											<div className="relative">
												<svg width="52" height="52" className="-rotate-90">
													<circle cx="26" cy="26" r={r} fill="none" stroke="var(--border-track)" strokeWidth="3.5" />
													<circle
														cx="26" cy="26" r={r}
														fill="none"
											...
											<div>
												<p className="text-xs font-medium" style={{ color: "var(--text-secondary)" }}>Güven</p>
												<p className="text-[10px]" style={{ color: "var(--text-faint)" }}>
													{confPct >= 80 ? "Yüksek" : confPct >= 60 ? "Orta" : "Düşük"}
												</p>
											</div>
										</div>
									)}
									...
									{liveResult.root_cause && (
										...
										>
											<div className="flex items-center gap-1.5 mb-2">
												<Target className="w-3.5 h-3.5" style={{ color: "var(--color-pink)" }} />
												<span className="text-xs font-medium" style={{ color: "var(--text-muted)" }}>Kök Neden</span>
											</div>
									...
									{liveResult.recommendations && liveResult.recommendations.length > 0 && (
										...
										>
											<div className="flex items-center gap-1.5 mb-2">
												...
												<span className="text-xs font-medium" style={{ color: "var(--text-muted)" }}>Öneriler</span>
											</div>
			...
			>
				{!serviceForInsights ? (
					...
					<div className="space-y-2">
						{[1, 2, 3].map((i) => (
							<Card
								...
								style={{ background: "var(--surface-card)", border: "1px solid var(--border-default)" }}
							>
								<div className="h-3.5 w-56 rounded mb-2" style={{ background: "var(--surface-sunken)" }} />
								<div className="h-2.5 w-36 rounded" style={{ background: "var(--surface-sunken)" }} />
							</Card>
						))}
					</div>
				) : filtered.length === 0 ? (
					<EmptyState
						...
						description={search || priorityFilter !== "all" ? "Filtre kriterlerini değiştirin." : '"Analiz Et" ile ilk analizi başlatın.'}
					/>
				) : (
					<>
						<div className="space-y-2">
