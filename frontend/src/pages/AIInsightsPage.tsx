...
export function AIInsightsPage() {
  ...
  const serviceForInsights = selectedServiceId === "all" ? (services[0]?.id ?? "") : selectedServiceId;
  ...
  const priorityBadgeStyle = (priority: string) => ({
    background: priority === "high" ? "var(--status-down-subtle)" : priority === "medium" ? "var(--status-warn-subtle)" : "var(--color-teal-subtle)",
    color: priority === "high" ? "var(--status-down-text)" : priority === "medium" ? "var(--status-warn-text)" : "var(--color-teal)",
    borderColor: priority === "high" ? "var(--status-down-border)" : priority === "medium" ? "var(--status-warn-border)" : "var(--color-teal-border)",
  });
    ...
    <div className="space-y-6">
      ...
      {allInsights.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.05 }}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Toplam", value: totalInsights, icon: BarChart3, color: "var(--color-lavender)", bg: "var(--color-lavender-subtle)", border: "var(--color-lavender-border)" },
              ...
              <Card key={label} className="p-3 rounded text-center" style={{ background: bg, border: `2px solid ${border}`, boxShadow: "var(--card-shadow)" }}>
        ...
        <Card className="p-4 rounded" style={{ background: "var(--surface-card)", border: "2px solid var(--color-lavender-border)", boxShadow: "var(--card-shadow)" }}>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold" style={{ color: "var(--text-secondary)" }}>Yeni Analiz Başlat</span>
            </div>
            <div className="flex flex-1 items-center gap-2 flex-wrap">
              <Select value={analyzeServiceId} onValueChange={setAnalyzeServiceId}>
                <SelectTrigger className="w-44 rounded text-xs h-8" style={{ background: "var(--input-bg)", borderColor: "var(--input-border)", color: "var(--text-secondary)" }}>
                  <SelectValue placeholder="Servis seçin..." />
                </SelectTrigger>
                <SelectContent className="rounded" style={{ background: "var(--surface-card)", borderColor: "var(--border-default)" }}>
                  {services.map((s) => (
              ...
              <div className="flex rounded overflow-hidden border-2" style={{ borderColor: "var(--color-lavender-border)" }}>
                <button onClick={() => setDeepAnalysis(false)}
                  className="px-3 py-1.5 text-[10px] font-medium transition-all"
                  style={!deepAnalysis ? { background: "var(--color-lavender-subtle)", color: "var(--color-lavender)" } : { color: "var(--text-muted)" }}>
                  Hızlı
                </button>
                <button onClick={() => setDeepAnalysis(true)}
                  className="px-3 py-1.5 text-[10px] font-medium transition-all"
                  style={deepAnalysis ? { background: "var(--color-lavender-subtle)", color: "var(--color-lavender)" } : { color: "var(--text-muted)" }}>
                  Derin
                </button>
      ...
      {analyzeMutation.isPending && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <Card className="rounded p-5 space-y-3" style={{ background: "var(--surface-card)", border: "2px solid var(--color-lavender-border)" }}>
            <div className="flex items-center gap-3 mb-2">
              <Brain className="w-5 h-5 animate-pulse" style={{ color: "var(--color-lavender)" }} />
              <div className="h-3 w-48 rounded animate-pulse" style={{ background: "var(--color-lavender-subtle)" }} />
            </div>
            {[80, 60, 90, 50].map((w, i) => (
              <div key={i} className="h-2 rounded animate-pulse" style={{ width: `${w}%`, animationDelay: `${i * 0.15}s`, background: "var(--color-lavender-subtle)" }} />
            ))}
          </Card>
        </motion.div>
      )}
      ...
      <AnimatePresence>
        {liveResult && !analyzeMutation.isPending && (
          <motion.div key="live-result" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.4 }}>
            <Card className="rounded overflow-hidden" style={{ background: "var(--surface-card)", border: "2px solid var(--color-lavender-border)", boxShadow: "var(--card-shadow)" }}>
              ...
              <div className="p-5 space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[10px]" style={{ color: "var(--text-faint)" }}>{deepAnalysis ? "Derin analiz" : "Hızlı analiz"} · Son 30 dakika</p>
                  </div>
                      ...
                      <ul className="space-y-2">
                        {liveResult.recommendations.map((rec, i) => (
                          <li key={i} className="flex items-start gap-2">
                            <Badge className="text-[9px] px-1.5 py-0 rounded shrink-0 mt-0.5 border-2" style={priorityBadgeStyle(rec.priority)}>{rec.priority}</Badge>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }}>
        <Card className="p-3 rounded" style={{ background: "var(--surface-card)", border: "2px solid var(--border-default)", boxShadow: "var(--card-shadow)" }}>
          <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
            ...
            <Select value={selectedServiceId} onValueChange={(v) => { setSelectedServiceId(v); setPage(0); }}>
              ...
              <SelectContent className="rounded" style={{ background: "var(--surface-card)", borderColor: "var(--border-default)" }}>
                <SelectItem value="all" className="text-xs">Tüm Servisler</SelectItem>
                {services.map((s) => (
            ...
            <div className="relative flex-1">
              ...
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="İçerik ara..."
            ...
            <div className="flex items-center gap-1">
              <Filter className="w-3 h-3 shrink-0" style={{ color: "var(--text-faint)" }} />
              {(["all", "high", "medium", "low"] as const).map((p) => (
                <button key={p} onClick={() => setPriorityFilter(p)}
                  ...
                  {p === "all" ? "Tümü" : p === "high" ? "Yüksek" : p === "medium" ? "Orta" : "Düşük"}
                </button>
              ))}
            </div>
          </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.25 }}>
        {!serviceForInsights ? (
          ...
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <Card key={i} className="p-4 rounded animate-pulse" style={{ background: "var(--surface-card)", border: "2px solid var(--border-default)" }}>
                <div className="h-4 w-64 rounded mb-2" style={{ background: "var(--color-lavender-subtle)" }} />
                <div className="h-3 w-40 rounded" style={{ background: "var(--color-lavender-subtle)" }} />
              </Card>
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <EmptyState
            icon={Brain}
            title={search || priorityFilter !== "all" ? "Sonuç bulunamadı" : "Henüz AI insight yok"}
            description={search || priorityFilter !== "all" ? "Arama veya filtre kriterlerinizi değiştirin." : "Yukarıdaki \"Analiz Et\" butonuyla ilk analizi başlatın."}
          />
        ) : (
          <>
            <div className="space-y-3">
                ...
                {filtered.map((insight: AIInsight, index: number) => {
                  ...
                  return (
                    <motion.div
                      ...
                      layout
                    >
                      <Card className="rounded overflow-hidden transition-all duration-200"
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-start gap-3 min-w-0">
                              <div className="w-8 h-8 rounded flex items-center justify-center shrink-0 mt-0.5"
                                style={{ backgroundColor: "var(--color-lavender-subtle)", border: "1.5px solid var(--color-lavender-border)" }}>
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                                  <span className="text-[10px] flex items-center gap-1" style={{ color: "var(--text-faint)" }}>
                                    <Clock className="w-3 h-3" />{new Date(insight.created_at).toLocaleString("tr-TR")}
                                  </span>
                                  ...
                                  {insight.recommendations && insight.recommendations.length > 0 && (
                                    <Badge className="text-[8px] px-1.5 py-0 rounded border-2" style={priorityBadgeStyle(topPriority)}>
                                      <Eye className="w-2 h-2 mr-0.5 inline" />{insight.recommendations.length} öneri
                                    </Badge>
                                  )}
                                </div>
                              </div>
                              ...
                              {insight.recommendations && insight.recommendations.length > 0 && (
                                <div className="p-3 rounded" style={{ background: "var(--color-teal-subtle)", border: "2px solid var(--color-teal-border)" }}>
                                  ...
                                  <ul className="space-y-1.5">
                                    {insight.recommendations.map((rec: { action: string; priority: string }, i: number) => (
                                      <li key={i} className="flex items-start gap-2">
                                        <Badge className="text-[8px] px-1 py-0 rounded-full shrink-0 mt-0.5 border" style={priorityBadgeStyle(rec.priority)}>
                                          {rec.priority}
                                        </Badge>
            ...
            {totalInsights > PAGE_SIZE && (
              <div className="flex items-center justify-between pt-2">
                <Button size="sm" variant="outline" onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
                  className="rounded text-xs h-8">Önceki</Button>
                ...
                <Button size="sm" variant="outline" onClick={() => setPage(p => p + 1)}
                  ...
                  className="rounded text-xs h-8">Sonraki</Button>
              </div>
            )}
          </>
        )}
