...
export const k8sApi = {
	...
	deletePod: async (name: string) => {
		const { data } = await apiClient.delete(
			`/k8s/pods/${encodeURIComponent(name)}`,
		);
	...
	scaleDeployment: async (name: string, replicas: number) => {
		const { data } = await apiClient.post(`/k8s/deployments/${name}/scale`, {
			replicas,
		});
	...
	undeployService: async (name: string) => {
		const { data } = await apiClient.delete(
			`/k8s/deploy/${encodeURIComponent(name)}`,
			{ timeout: 30000 },
		);
		return data.data as { message: string };
	},
