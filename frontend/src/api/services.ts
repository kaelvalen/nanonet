...
export const servicesApi = {
	...
	exec: async (
		...
		command: string,
		timeoutSec = 30,
	): Promise<{ command_id: string; status: string; queued_at: string }> => {
		const response = await apiClient.post(`/services/${id}/exec`, {
			command,
	...
	scale: async (
		...
		const response = await apiClient.post(`/services/${id}/scale`, {
			instances,
			strategy,
		});
		return response.data.data;
	...
	getCommandHistory: async (
		...
		limit = 20,
		page = 1,
	): Promise<{ commands: CommandLog[]; total: number; page: number }> => {
		const response = await apiClient.get(`/services/${id}/commands`, {
			params: { limit, page },
		});
		return response.data.data;
	},
