...
import { Terminal, Layers, RefreshCw, Minus, Plus, Loader2, TrendingUp, Activity } from "lucide-react";
...
export function LoadBalancingTab({
  ...
  scaleInstances, setScaleInstances,
  scaleStrategy, setScaleStrategy,
  scaleLoading, handleScale,
}: LoadBalancingTabProps) {
  const queryClient = useQueryClient();
  const [k8sMode, setK8sMode] = React.useState<'k8s' | 'agent'>('k8s');
  const [k8sDeployName, setK8sDeployName] = React.useState(serviceName.toLowerCase().replace(/\s+/g, '-'));
  ...
  const k8sScaleMutation = useMutation({
    ...
    onSuccess: (r) => { toast.success(r.message); refetchDep(); },
    onError: () => toast.error("K8s scale başarısız"),
  });

  const hpaMutation = useMutation({
    mutationFn: () => k8sApi.createOrUpdateHPA(k8sDeployName, hpaMin, hpaMax, hpaCpu),
    onSuccess: () => { toast.success("HPA uygulandı"); refetchHPA(); },
    onError: () => toast.error("HPA oluşturulamadı"),
  });

  const deleteHPAMutation = useMutation({
    mutationFn: () => k8sApi.deleteHPA(k8sDeployName),
    onSuccess: () => { toast.success("HPA silindi"); refetchHPA(); },
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          Kubernetes
          {isK8sAvailable && <span className="w-1.5 h-1.5 rounded-full ml-1" style={{ backgroundColor: "var(--status-up)" }} />}
          {!isK8sAvailable && <span className="text-[9px] ml-1" style={{ color: "var(--text-faint)" }}>(devre dışı)</span>}
        </button>
        <button
          ...
          {!isK8sAvailable ? (
            <Card className="p-8 rounded text-center" style={{ background: "var(--surface-card)", border: "2px solid var(--color-blue-border)", boxShadow: "var(--card-shadow)" }}>
              ...
              <p className="text-[10px] max-w-sm mx-auto" style={{ color: "var(--text-muted)" }}>
                Backend'de <code className="px-1 rounded" style={{ background: "var(--surface-sunken)" }}>K8S_NAMESPACE</code> tanımlayarak
                Kubernetes entegrasyonunu aktifleştirebilirsiniz.
              </p>
              ...
              {deployment && (
                <Card className="p-5 rounded" style={{ background: "var(--surface-card)", border: "2px solid var(--color-blue-border)", boxShadow: "var(--card-shadow)" }}>
                  ...
                  <div className="grid grid-cols-4 gap-2 mb-4">
                    {[
                      { label: "İstenen", value: deployment.replicas, color: "var(--color-blue)" },
                      { label: "Hazır", value: deployment.ready_replicas, color: "var(--status-up)" },
                ...
                <Card className="p-4 rounded" style={{ background: "var(--surface-card)", border: "2px solid var(--color-teal-border)", boxShadow: "var(--card-shadow)" }}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Badge className="text-[9px] px-1.5 py-0.5 rounded border-2"
                      ...
                      {pods.filter((p: any) => p.ready).length}/{pods.length} hazır
                    </Badge>
                  </div>
                  <div className="space-y-1.5">
                    {pods.map((pod: any) => (
                      <div key={pod.name} className="flex items-center gap-2 px-3 py-2 rounded" style={{ background: "var(--surface-sunken)", border: "1.5px solid var(--border-default)" }}>
                        ...
                        {pod.restarts > 0 && (
                          <span className="text-[9px] px-1.5 py-0.5 rounded-full" style={{ background: "var(--status-warn-subtle)", color: "var(--status-warn-text)" }}>
                            {pod.restarts}↻
                          </span>
                        )}
                        <Badge className="text-[8px] px-1 py-0 rounded shrink-0"
                          style={pod.status === "Running"
                            ? { background: "var(--status-up-subtle)", color: "var(--status-up-text)" }
                            : { background: "var(--status-down-subtle)", color: "var(--status-down-text)" }}>
                          {pod.status}
                        </Badge>
                      </div>
                    ))}
              ...
              {deployment && (
                <Card className="p-5 rounded" style={{ background: "var(--surface-card)", border: "2px solid var(--color-lavender-border)", boxShadow: "var(--card-shadow)" }}>
                  ...
                  {hpaInfo && (
                    <div className="grid grid-cols-4 gap-2 mb-4">
                      {[
                        { label: "Min", value: (hpaInfo as any).min_replicas, color: "var(--color-lavender)" },
                        { label: "Max", value: (hpaInfo as any).max_replicas, color: "var(--color-lavender)" },
                        { label: "Mevcut", value: (hpaInfo as any).current_replicas, color: "var(--color-blue)" },
                        { label: "CPU Hedef", value: (hpaInfo as any).cpu_target_percent ? `%${(hpaInfo as any).cpu_target_percent}` : "-", color: "var(--status-up)" },
                      ].map((s) => (
                  ...
                  <Button onClick={() => hpaMutation.mutate()} disabled={hpaMutation.isPending}
                    ...
                    {hpaMutation.isPending
                      ...
                      : <><TrendingUp className="w-3 h-3 mr-1" /> {hpaInfo ? "HPA Güncelle" : "HPA Oluştur"}</>}
                  </Button>
                </Card>
              )}
            </>
      ...
      {k8sMode === 'agent' && (
        <Card className="p-6 rounded" style={{ background: "var(--surface-card)", border: "2px solid var(--status-up-border)", boxShadow: "var(--card-shadow)" }}>
          ...
          <div className="space-y-4">
            ...
            {/* Strategy */}
            <div>
              <label className="text-[10px] uppercase tracking-wider block mb-2" style={{ color: "var(--text-muted)" }}>LB Stratejisi</label>
              <div className="grid grid-cols-3 gap-2">
                {([
                  { value: 'round_robin' as const, label: 'Round Robin', desc: 'Sırayla dağıt' },
                  { value: 'least_conn' as const, label: 'Least Conn', desc: 'En az bağlantı' },
