...
function CopyBox({ value, label }: { value: string; label?: string }) {
  ...
  return (
    ...
    >
      {label && (
        <p className="text-[10px] mb-1 font-sans font-semibold" style={{ color: "var(--text-muted)" }}>
          {label}
        </p>
      )}
      <pre className="whitespace-pre-wrap break-all pr-8" style={{ color: "var(--text-secondary)" }}>
        {value}
      </pre>
      <button
        onClick={copy}
        ...
        style={{
          background: copied ? "var(--status-up-subtle)" : "var(--surface-card)",
...
export function AgentSetupWizard({ open, onClose, serviceId, serviceName }: AgentSetupWizardProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-sm font-bold" style={{ color: "var(--text-primary)" }}>
        ...
        {step === 1 && (
          <div className="space-y-4">
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>
              Agent'ın backend'e bağlanabilmesi için bir kimlik doğrulama token'ı gerekir.
              Bu token yalnızca bu servis için geçerlidir.
            </p>
            <div
            ...
            >
              <strong>Önemli:</strong> Token yalnızca bir kez gösterilir. Güvenli bir yere kaydedin.
            </div>
            <Button
            ...
            >
              {generateToken.isPending ? (
                <><Loader2 className="w-3.5 h-3.5 mr-2 animate-spin" /> Oluşturuluyor...</>
              ) : (
        ...
        {step === 2 && agentToken && (
          <div className="space-y-3">
            ...
            <CopyBox value={agentToken} label="Agent Token (güvenli yere kaydedin)" />
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center">
                <span className="px-2 text-[10px]" style={{ background: "var(--surface-card)", color: "var(--text-faint)" }}>
                  veya Docker ile
                </span>
              </div>
            </div>
            ...
            >
              Agent başarıyla bağlandığında bu servisin durumu <strong>UP</strong> olarak güncellenecektir.
            </div>
