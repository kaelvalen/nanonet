...
function DeletePodButton({
	...
	podName: string;
	deletePodMutation: UseMutationResult<
		{ message: string },
		unknown,
		string,
		unknown
		...
		>
			...
			{allPodsLoading && pods.length === 0 ? (
				...
				>
					...
					<span className="text-xs">Pod'lar yükleniyor...</span>
				</div>
				...
				>
					...
					<p className="text-xs" style={{ color: "var(--text-muted)" }}>
						{podFilter
							? "Filtrele uyan pod bulunamadı"
							: "Namespace'de pod yok"}
					</p>
				</Card>
			)}
		</motion.div>
