...
import { Sparkles, ChevronRight, Search, Server, AlertCircle, Settings, PanelLeftClose, PanelLeft } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function FloatingStatusBar({ onOpenCommandPalette }: { onOpenCommandPalette: () => void }) {
  ...
  const { navMode, toggleNavMode } = useNavStore();

  const crumbs = buildBreadcrumbs(location.pathname);
  const isHome = location.pathname === "/";
  const isSidebar = navMode === "sidebar";

  const initials = user?.email ? user.email.substring(0, 2).toUpperCase() : "NN";
  const upCount = services.filter((s) => s.status === "up").length;
  const downCount = services.filter((s) => s.status === "down" || s.status === "degraded").length;

  /* ── SIDEBAR MODE: sticky top bar with breadcrumbs ── */
  if (isSidebar) {
    return (
        ...
        >
            ...
            {crumbs.map((crumb, i) => (
              <span key={crumb.path} className="flex items-center gap-1 min-w-0">
                ...
                {i === crumbs.length - 1 ? (
                  ...
                  <Link to={crumb.path} className="text-xs transition-colors truncate hover:underline" style={{ color: "var(--text-muted)" }}>
                    {crumb.label}
                  </Link>
                )}
              </span>
          ...
          {downCount > 0 && (
            <button
              ...
              style={{ background: "var(--status-down-subtle)", border: "1.5px solid var(--status-down-border)", color: "var(--status-down-text)" }}
            >
          ...
          >
            ...
            <kbd className="text-[10px] px-1.5 py-0.5 rounded ml-2" style={{ background: "var(--surface-raised)", border: "1px solid var(--border-default)", color: "var(--text-faint)" }}>⌘K</kbd>
          </button>
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-2 py-1 rounded"
            ...
            <span className="text-[10px] font-bold" style={{ color: isConnected ? "var(--status-up-text)" : "var(--status-down-text)" }}>
              {isConnected ? "LIVE" : "OFF"}
            </span>
          </div>

          <div className="hidden lg:flex items-center gap-1.5 px-2 py-1 rounded" style={{ background: "var(--color-ai-subtle)", border: "1.5px solid var(--color-ai-border)" }}>
          ...
          <button
            ...
            style={{ background: "var(--surface-sunken)", border: "2px solid var(--border-default)", color: "var(--text-muted)" }}
          >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="relative group shrink-0">
                <div className="w-7 h-7 rounded flex items-center justify-center" style={{ background: "var(--gradient-logo)", border: "2px solid var(--border-default)", boxShadow: "var(--btn-shadow)" }}>
                  <span className="text-white text-xs font-bold">{initials}</span>
                </div>
                {isConnected && (
                  <div className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full" style={{ backgroundColor: "var(--status-up)", border: "1.5px solid var(--surface-card)" }} />
                )}
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="rounded" style={{ background: "var(--surface-card)", border: "2px solid var(--border-default)", boxShadow: "var(--card-shadow)" }}>
              ...
              <DropdownMenuItem className="text-xs cursor-pointer" style={{ color: "var(--text-secondary)" }} onClick={() => navigate("/settings")}>Ayarlar</DropdownMenuItem>
              <DropdownMenuSeparator style={{ backgroundColor: "var(--border-subtle)" }} />
              <DropdownMenuItem className="text-xs cursor-pointer" style={{ color: "var(--text-danger)" }} onClick={() => logout()}>Çıkış Yap</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </header>
  ...
  /* ── FLOATING MODE: original beautiful pill bar ── */
  return (
    <header className="fixed top-4 left-1/2 -translate-x-1/2 z-40 w-[calc(100%-2rem)] max-w-5xl">
      <div className="relative">
        <div className="relative rounded px-4 py-2 flex items-center gap-3" style={{ background: "var(--surface-card)", border: "2px solid var(--border-default)", boxShadow: "var(--card-shadow)" }}>
          ...
          {/* Logo */}
          <button onClick={() => navigate("/")} className="flex items-center gap-2 group cursor-pointer shrink-0">
              ...
              {crumbs.map((crumb, i) => (
                <span key={crumb.path} className="flex items-center gap-1 min-w-0">
                  ...
                  {i === crumbs.length - 1 ? (
                    ...
                    <Link to={crumb.path} className="text-xs transition-colors truncate max-w-20" style={{ color: "var(--text-muted)" }}>{crumb.label}</Link>
                  )}
                </span>
              ))}
            </nav>
          ...
          {isHome && (
            <nav className="hidden lg:flex items-center gap-1">
              {[
                ...
                <Link key={path} to={path} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] transition-all" style={{ color: "var(--text-muted)" }}>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="rounded" style={{ background: "var(--surface-card)", border: "2px solid var(--border-default)", boxShadow: "var(--card-shadow)" }}>
              <DropdownMenuLabel className="text-xs" style={{ color: "var(--text-secondary)" }}>{user?.email || "My Account"}</DropdownMenuLabel>
              <DropdownMenuSeparator style={{ backgroundColor: "var(--border-subtle)" }} />
              <DropdownMenuItem className="text-xs cursor-pointer" style={{ color: "var(--text-secondary)" }} onClick={() => navigate("/settings")}>Settings</DropdownMenuItem>
              <DropdownMenuSeparator style={{ backgroundColor: "var(--border-subtle)" }} />
              <DropdownMenuItem className="text-xs cursor-pointer" style={{ color: "var(--text-danger)" }} onClick={() => logout()}>Logout</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
