...
export function FloatingStatusBar({
	...
	return (
		<header className="fixed top-4 left-1/2 -translate-x-1/2 z-40 w-[calc(100%-2rem)] max-w-5xl">
			<div className="relative">
				<div
					className="relative rounded px-4 py-2 flex items-center gap-3"
					style={{
						background: "var(--surface-card)",
						border: "2px solid var(--border-default)",
						boxShadow: "var(--card-shadow)",
				...
				>
						...
						<nav className="hidden sm:flex items-center gap-1 min-w-0">
							{crumbs.map((crumb, i) => (
								...
								>
									...
									{i === crumbs.length - 1 ? (
										<span
											className="text-xs font-(--font-mono) truncate max-w-28"
						...
						<div className="hidden sm:flex items-center gap-2">
							...
							>
								...
								<span
									...
									style={{ color: "var(--color-teal-hover)" }}
								>
									{services.length}
								</span>
							</div>
					...
					>
						<Sparkles
							className="w-3 h-3 animate-glow"
							style={{ color: "var(--color-ai)" }}
						/>
						<div className="flex gap-0.5">
							<div
								className="w-0.5 h-2 rounded-sm animate-pulse"
								style={{
							...
							/>
							<div
								className="w-0.5 h-3 rounded-sm animate-pulse"
								style={{
									backgroundColor: "var(--color-lavender)",
									animationDelay: "0.2s",
								}}
							/>
							<div
								className="w-0.5 h-1.5 rounded-sm animate-pulse"
								style={{
									backgroundColor: "var(--color-lavender)",
									animationDelay: "0.4s",
								}}
					...
					<DropdownMenu>
						<DropdownMenuTrigger asChild>
							<button className="relative group shrink-0">
								...
								>
									<span className="text-white text-xs font-(--font-quicksand)">
										{initials}
									</span>
								</div>
								{isConnected && (
